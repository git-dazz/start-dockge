...
*/
import {ComposeTask} from "/app/data/compose-task.js";
export class Stack {
    ...
    async deploy(socket : DockgeSocket) : Promise<number> {
        ...
        let exitCode = await Terminal.exec(this.server, socket, terminalName, "docker", ComposeTask.deploy, this.path);
        if (exitCode !== 0) {
    ...
    async start(socket: DockgeSocket) {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
        // let exitCode = await Terminal.exec(this.server, socket, terminalName, "docker", [ "compose", "up", "-d", "--remove-orphans" ], this.path);
        let exitCode = await Terminal.exec(this.server, socket, terminalName, "docker", ComposeTask.start, this.path);
        if (exitCode !== 0) {
    ...
    async update(socket: DockgeSocket) {
        ...
        exitCode = await Terminal.exec(this.server, socket, terminalName, "docker", ComposeTask.update, this.path);
        if (exitCode !== 0) {
            throw new Error("Failed to restart, please check the terminal output for more information.");
        }
        return exitCode;
